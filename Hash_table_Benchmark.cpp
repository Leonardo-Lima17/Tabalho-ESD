#define TABLE_SIZE 10007 // Tamanho da tabela hash para transacoes

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdbool.h>
#include <math.h>
#include <stdint.h>
#include <windows.h> // Necessario para HighPrecisionTimer e Sleep
#include <time.h>    // Necessario para srand, time

#ifdef _MSC_VER
#define _CRT_SECURE_NO_WARNINGS
#endif

#define NUM_REPETITIONS 10 // Numero de repeticoes para cada benchmark individual

// ================= ESTRUTURAS DE DADOS =================

// Estrutura para representar uma transacao
typedef struct Transaction {
    char transaction_id[16];
    char timestamp[32];
    char sender_account[32];
    char receiver_account[32];
    float amount;
    char transaction_type[16];
    char merchant_category[32];
    char location[32];
    char device_used[16];
    bool is_fraud;
    struct Transaction* next; // Ponteiro para o proximo no em caso de colisao (lista encadeada)
} Transaction;

// Estrutura para simular MachineData para o printf
typedef struct MachineData_Simulated {
    int UDI;
    char ProductID[10];
    char Type;
    float AirTemp;
    float ProcessTemp;
    int RotationalSpeed;
    float Torque;
    int ToolWear;
    bool MachineFailure;
    bool TWF;
    bool HDF;
    bool PWF;
    bool OSF;
    bool RNF;
} MachineData_Simulated;


// Estrutura de um no da Arvore AVL (adicionada para o printf de sizeof(AVLNode))
// Esta estrutura e usada apenas para exibir seu tamanho na saida do benchmark,
// nao e parte da logica da Hash Table.
typedef struct AVLNode {
    int key; 
    Transaction data;
    struct AVLNode* left;
    struct AVLNode* right;
    uint8_t height;
} AVLNode;


// Estrutura da Tabela Hash
typedef struct HashTable {
    Transaction** table; // Array de ponteiros para Transaction (cada um o inicio de uma lista)
    int capacity;        // Capacidade do array da tabela hash
    int size;            // Numero total de elementos armazenados na tabela
} HashTable;

// ================= FUNCOES DA TABELA HASH =================

// Funcao de hash simples (DJB2)
unsigned int hash(const char* str) {
    unsigned int hash_val = 5381;
    int c;
    while ((c = *str++))
        hash_val = ((hash_val << 5) + hash_val) + c; 
    return hash_val % TABLE_SIZE;
}

// Inicializa a tabela hash
void init_hash_table(HashTable* ht) {
    ht->table = (Transaction**)calloc(TABLE_SIZE, sizeof(Transaction*));
    if (!ht->table) {
        fprintf(stderr, "Erro ao alocar memoria para a tabela hash.\n");
        exit(EXIT_FAILURE);
    }
    ht->capacity = TABLE_SIZE;
    ht->size = 0;
}

// Libera toda a memoria alocada pela tabela hash
void free_hash_table(HashTable* ht) {
    for (int i = 0; i < ht->capacity; i++) {
        Transaction* current = ht->table[i];
        while (current) {
            Transaction* temp = current;
            current = current->next;
            free(temp);
        }
    }
    free(ht->table);
    ht->table = NULL;
    ht->capacity = 0;
    ht->size = 0;
}

// Insere uma transacao na tabela hash
void insert_transaction(HashTable* ht, Transaction t) {
    unsigned int index = hash(t.transaction_id);
    Transaction* new_node = (Transaction*)malloc(sizeof(Transaction));
    if (!new_node) {
        fprintf(stderr, "Erro de alocacao de memoria para novo no.\n");
        exit(EXIT_FAILURE);
    }
    *new_node = t; 
    new_node->next = ht->table[index]; 
    ht->table[index] = new_node;
    ht->size++; 
}

// Busca uma transacao na tabela hash pelo transaction_id
Transaction* search_transaction(HashTable* ht, const char* transaction_id) {
    unsigned int index = hash(transaction_id);
    Transaction* current = ht->table[index];
    while (current) {
        if (strcmp(current->transaction_id, transaction_id) == 0)
            return current;
        current = current->next;
    }
    return NULL;
}

// Remove uma transacao da tabela hash pelo transaction_id
void remove_transaction(HashTable* ht, const char* transaction_id) {
    unsigned int index = hash(transaction_id);
    Transaction* current = ht->table[index];
    Transaction* prev = NULL;

    while (current) {
        if (strcmp(current->transaction_id, transaction_id) == 0) {
            if (prev) {
                prev->next = current->next;
            } else {
                ht->table[index] = current->next;
            }
            free(current);
            ht->size--;
            return;
        }
        prev = current;
        current = current->next;
    }
}


// ================= FUNCOES AUXILIARES PARA ESTATISTICAS =================

// Calcula a media de um array de doubles
double calculate_mean(double* data, int count) {
    double sum = 0.0;
    for (int i = 0; i < count; i++) {
        sum += data[i];
    }
    return sum / count;
}

// Calcula o desvio padrao de um array de doubles
double calculate_std_dev(double* data, int count, double mean) {
    double sum_sq_diff = 0.0;
    for (int i = 0; i < count; i++) {
        sum_sq_diff += pow(data[i] - mean, 2);
    }
    return sqrt(sum_sq_diff / count);
}

// Calcula o coeficiente de variacao
double calculate_coeff_of_variation(double mean, double std_dev) {
    if (mean == 0) return 0.0; // Evita divisao por zero
    return (std_dev / mean) * 100.0; // Em porcentagem
}

// ================= FUNCOES DE BENCHMARK =================

// Estrutura para timer de alta precisao (Windows specific)
typedef struct {
    LARGE_INTEGER start;
    LARGE_INTEGER end;
    LARGE_INTEGER frequency;
} HighPrecisionTimer;

// Inicia o timer
void start_timer(HighPrecisionTimer* timer) {
    QueryPerformanceFrequency(&timer->frequency);
    QueryPerformanceCounter(&timer->start);
}

// Para o timer e retorna o tempo decorrido em milissegundos
double stop_timer(HighPrecisionTimer* timer) {
    QueryPerformanceCounter(&timer->end);
    double elapsed = (double)(timer->end.QuadPart - timer->start.QuadPart) * 1000.0 / timer->frequency.QuadPart;
    return elapsed;
}

// Gera dados aleatorios e os insere na HashTable
void generateRandomDataForHash(HashTable* ht, int count) {
    for (int i = 0; i < count; i++) {
        Transaction d = {0}; 
        // Gera transaction_id unico como string
        snprintf(d.transaction_id, sizeof(d.transaction_id), "TRN%08d%04d", i, rand() % 10000); 
        
        snprintf(d.timestamp, sizeof(d.timestamp), "2024-01-01 %02d:%02d:%02d", rand() % 24, rand() % 60, rand() % 60);
        snprintf(d.sender_account, sizeof(d.sender_account), "ACC%05d", rand() % 100000);
        snprintf(d.receiver_account, sizeof(d.receiver_account), "ACC%05d", rand() % 100000);
        d.amount = 10.0f + (rand() % 10000) / 100.0f;
        char types[][16] = {"purchase", "transfer", "deposit", "withdrawal", "payment"};
        snprintf(d.transaction_type, sizeof(d.transaction_type), "%s", types[rand() % 5]);
        char categories[][32] = {"retail", "food", "travel", "online", "services", "utilities", "healthcare"};
        snprintf(d.merchant_category, sizeof(d.merchant_category), "%s", categories[rand() % 7]);
        snprintf(d.location, sizeof(d.location), "City%03d", rand() % 500);
        char devices[][16] = {"mobile", "web", "pos", "atm"};
        snprintf(d.device_used, sizeof(d.device_used), "%s", devices[rand() % 4]);
        d.is_fraud = (rand() % 100 < 5); // 5% de chance de ser fraude

        insert_transaction(ht, d);
    }
}

// Benchmark de insercao da HashTable
void benchmark_insertion_hashtable(int num_elements) {
    double results[NUM_REPETITIONS];
    printf("\nBenchmark Insercao (%d elementos):\n", num_elements);

    for (int i = 0; i < NUM_REPETITIONS; i++) {
        HighPrecisionTimer t;
        HashTable tmp_ht;
        init_hash_table(&tmp_ht); // Sempre inicia com uma tabela hash limpa
        
        start_timer(&t);
        generateRandomDataForHash(&tmp_ht, num_elements);
        results[i] = stop_timer(&t);
        
        free_hash_table(&tmp_ht); // Libera memoria da tabela hash temporaria
    }

    double mean = calculate_mean(results, NUM_REPETITIONS);
    double std_dev = calculate_std_dev(results, NUM_REPETITIONS, mean);
    double cov = calculate_coeff_of_variation(mean, std_dev);

    printf("  Media: %.3f ms\n", mean);
    printf("  Desvio Padrao: %.3f ms\n", std_dev);
    printf("  Coeficiente de Variacao: %.2f%%\n", cov);
}

// Benchmark de busca da HashTable
void benchmark_search_hashtable(HashTable* ht) {
    if (ht->size == 0) {
        printf("Tabela Hash vazia para busca\n");
        return;
    }

    double results[NUM_REPETITIONS];
    printf("\nBenchmark Busca (%d ops):\n", 10000); // 10000 ops e o numero de buscas por repeticao

    // Coleta as transaction_ids existentes para buscar (fora do loop de repeticoes para consistencia)
    char** ids_to_search = (char**)malloc(ht->size * sizeof(char*));
    if (!ids_to_search) {
        perror("Falha ao alocar memoria para IDs de busca");
        return;
    }
    int id_idx = 0;
    for (int i = 0; i < ht->capacity; i++) {
        Transaction* current = ht->table[i];
        while (current) {
            if (id_idx < ht->size) {
                ids_to_search[id_idx] = (char*)malloc(sizeof(current->transaction_id));
                strcpy(ids_to_search[id_idx], current->transaction_id);
                id_idx++;
            }
            current = current->next;
        }
    }

    for (int i = 0; i < NUM_REPETITIONS; i++) {
        HighPrecisionTimer t;
        const int searches_per_run = 10000;
        int found = 0;
        
        start_timer(&t);
        for (int j = 0; j < searches_per_run; j++) {
            // Busca por um ID aleatorio do conjunto de IDs existentes
            const char* search_id = ids_to_search[rand() % ht->size];
            if (search_transaction(ht, search_id) != NULL) {
                found++;
            }
        }
        results[i] = stop_timer(&t);
    }

    double mean = calculate_mean(results, NUM_REPETITIONS);
    double std_dev = calculate_std_dev(results, NUM_REPETITIONS, mean);
    double cov = calculate_coeff_of_variation(mean, std_dev);

    printf("  Media: %.3f ms\n", mean);
    printf("  Desvio Padrao: %.3f ms\n", std_dev);
    printf("  Coeficiente de Variacao: %.2f%%\n", cov);

    // Libera a memoria dos IDs coletados
    for (int i = 0; i < id_idx; i++) {
        free(ids_to_search[i]);
    }
    free(ids_to_search);
}

// Benchmark de remocao da HashTable
void benchmark_removal_hashtable(HashTable* ht) {
    if (ht->size == 0) {
        printf("Tabela Hash vazia para remocao\n");
        return;
    }

    double results[NUM_REPETITIONS];
    printf("\nBenchmark Remocao (%d ops):\n", 1000); // 1000 ops e o numero de remocoes por repeticao

    for (int i = 0; i < NUM_REPETITIONS; i++) {
        HighPrecisionTimer t;
        HashTable tmp_ht;
        init_hash_table(&tmp_ht);

        // Copia elementos da tabela original para uma tabela temporaria para remocao
        for (int j = 0; j < ht->capacity; j++) {
            Transaction* current = ht->table[j];
            while (current) {
                insert_transaction(&tmp_ht, *current); 
                current = current->next;
            }
        }

        const int removals_per_run = (tmp_ht.size < 1000) ? tmp_ht.size / 2 : 1000;
        if (removals_per_run == 0 && tmp_ht.size > 0) {
            // Nao ha remocoes suficientes para 1000 ops, ajusta.
        }

        // Coleta as transaction_ids para remocao da tmp_ht
        char** ids_to_remove = (char**)malloc(tmp_ht.size * sizeof(char*));
        if (!ids_to_remove) {
            perror("Falha ao alocar memoria para IDs de remocao");
            free_hash_table(&tmp_ht);
            return;
        }
        int id_idx = 0;
        for (int j = 0; j < tmp_ht.capacity; j++) {
            Transaction* current = tmp_ht.table[j];
            while (current) {
                if (id_idx < tmp_ht.size) { 
                    ids_to_remove[id_idx] = (char*)malloc(sizeof(current->transaction_id));
                    strcpy(ids_to_remove[id_idx], current->transaction_id);
                    id_idx++;
                }
                current = current->next;
            }
        }
        // Embaralha os IDs para remocao aleatoria
        for (int k = id_idx - 1; k > 0; k--) {
            int l = rand() % (k + 1);
            char* temp = ids_to_remove[k];
            ids_to_remove[k] = ids_to_remove[l];
            ids_to_remove[l] = temp;
        }

        start_timer(&t);
        for (int j = 0; j < removals_per_run; j++) {
            if (tmp_ht.size > 0 && j < id_idx) { 
                remove_transaction(&tmp_ht, ids_to_remove[j]);
            }
        }
        results[i] = stop_timer(&t);

        // Libera a memoria dos IDs coletados
        for (int k = 0; k < id_idx; k++) {
            free(ids_to_remove[k]);
        }
        free(ids_to_remove);
        free_hash_table(&tmp_ht); // Libera a tabela hash temporaria apos cada repeticao
    }

    double mean = calculate_mean(results, NUM_REPETITIONS);
    double std_dev = calculate_std_dev(results, NUM_REPETITIONS, mean);
    double cov = calculate_coeff_of_variation(mean, std_dev);

    printf("  Media: %.3f ms\n", mean);
    printf("  Desvio Padrao: %.3f ms\n", std_dev);
    printf("  Coeficiente de Variacao: %.2f%%\n", cov);
}

// Estima o uso total de memoria da HashTable
void estimate_memory_usage_hashtable(HashTable* ht) {
    if (ht == NULL) {
        printf("Tabela Hash invalida\n");
        return;
    }
    
    size_t node_size = sizeof(Transaction); 
    
    size_t total_nodes_memory = ht->size * node_size;
    size_t table_array_memory = ht->capacity * sizeof(Transaction*);
    size_t hashtable_struct_memory = sizeof(HashTable);

    size_t total_memory = total_nodes_memory + table_array_memory + hashtable_struct_memory;

    printf("\n=== ESTIMATIVA DE USO DE MEMORIA ===\n");
    printf("Tamanho por no: %zu bytes\n", node_size); 
    printf("Numero de nos: %d\n", ht->size); 
    printf("Memoria total estimada: %zu bytes (%.2f KB)\n",
           total_memory, (float)total_memory / 1024);

    printf("\nComparacao com sizeof:\n");
    printf("sizeof(MachineData): %zu bytes\n", sizeof(Transaction)); 
    printf("sizeof(AVLNode): %zu bytes\n", sizeof(AVLNode)); 
    printf("Obs: Pode haver padding/alignment pelo compilador\n");
}

// Benchmark de acesso aleatorio da HashTable
void benchmark_random_access_hashtable(HashTable* ht) {
    if (ht->size == 0) {
        printf("Tabela Hash vazia para teste de acesso aleatorio\n");
        return;
    }

    double results[NUM_REPETITIONS];
    printf("\nBenchmark Acesso Aleatorio (%d acessos):\n", 10000); // 10000 acessos por repeticao

    // Coleta as transaction_ids existentes para acesso aleatorio (fora do loop de repeticoes)
    char** ids_for_access = (char**)malloc(ht->size * sizeof(char*));
    if (!ids_for_access) {
        perror("Falha ao alocar memoria para IDs de acesso");
        return;
    }
    int id_idx = 0;
    for (int i = 0; i < ht->capacity; i++) {
        Transaction* current = ht->table[i];
        while (current) {
            if (id_idx < ht->size) {
                ids_for_access[id_idx] = (char*)malloc(sizeof(current->transaction_id));
                strcpy(ids_for_access[id_idx], current->transaction_id);
                id_idx++;
            }
            current = current->next;
        }
    }

    for (int i = 0; i < NUM_REPETITIONS; i++) {
        HighPrecisionTimer t;
        const int accesses_per_run = 10000;
        long long sum = 0; 

        start_timer(&t);
        for (int j = 0; j < accesses_per_run; j++) {
            const char* random_id = ids_for_access[rand() % ht->size];
            Transaction* found_tx = search_transaction(ht, random_id); 
            if (found_tx != NULL) {
                sum += (long long)found_tx->transaction_id[0]; 
            }
        }
        results[i] = stop_timer(&t);
    }

    double mean = calculate_mean(results, NUM_REPETITIONS);
    double std_dev = calculate_std_dev(results, NUM_REPETITIONS, mean);
    double cov = calculate_coeff_of_variation(mean, std_dev);

    printf("  Media: %.3f ms\n", mean);
    printf("  Desvio Padrao: %.3f ms\n", std_dev);
    printf("  Coeficiente de Variacao: %.2f%%\n", cov);

    // Libera a memoria dos IDs coletados
    for (int i = 0; i < id_idx; i++) {
        free(ids_for_access[i]);
    }
    free(ids_for_access);
}

// Benchmark de escalabilidade da HashTable para insercao
void benchmark_scalability_hashtable() {
    printf("\nBenchmark de Escalabilidade:\n");
    int sizes[] = {1000, 5000, 10000, 20000, 50000, 100000};
    int num_sizes = sizeof(sizes) / sizeof(sizes[0]);

    for (int s = 0; s < num_sizes; s++) { // Loop pelos tamanhos
        int current_size = sizes[s];
        double results[NUM_REPETITIONS];

        for (int i = 0; i < NUM_REPETITIONS; i++) { // Repeticoes para cada tamanho
            HashTable ht;
            init_hash_table(&ht);

            HighPrecisionTimer t;
            start_timer(&t);
            generateRandomDataForHash(&ht, current_size);
            results[i] = stop_timer(&t);
            free_hash_table(&ht);
        }
        double mean = calculate_mean(results, NUM_REPETITIONS);
        double std_dev = calculate_std_dev(results, NUM_REPETITIONS, mean);
        double cov = calculate_coeff_of_variation(mean, std_dev);

        printf("  Tamanho: %6d elementos | Media: %7.3f ms | DP: %7.3f ms | CV: %.2f%%\n",
               current_size, mean, std_dev, cov);
    }
}

// Benchmark de operacoes combinadas (insercao, busca, remocao) na HashTable
void benchmark_combined_operations_hashtable(HashTable* ht) {
    double results[NUM_REPETITIONS];
    printf("\nBenchmark Operacoes Combinadas (%d ops):\n", 1000); // 1000 ops por repeticao

    for (int i = 0; i < NUM_REPETITIONS; i++) {
        HighPrecisionTimer t;
        const int num_operations_per_run = 1000;
        
        // Crie uma copia da tabela hash para cada repeticao para garantir o estado inicial
        HashTable tmp_ht_for_ops;
        init_hash_table(&tmp_ht_for_ops);
        for (int j = 0; j < ht->capacity; j++) {
            Transaction* current = ht->table[j];
            while(current) {
                insert_transaction(&tmp_ht_for_ops, *current);
                current = current->next;
            }
        }

        start_timer(&t);
        for (int j = 0; j < num_operations_per_run; j++) {
            int op_type = rand() % 100; // 0-99
            if (op_type < 30) { // 30% Insercao
                Transaction d = {0};
                snprintf(d.transaction_id, sizeof(d.transaction_id), "TRN%08d%04d", j + (i * num_operations_per_run) + 1000000, rand() % 10000); 
                d.amount = 10.0f + (rand() % 10000) / 100.0f;
                insert_transaction(&tmp_ht_for_ops, d);
            } else if (op_type < 80) { // 50% Busca (30-79)
                char search_id[16];
                snprintf(search_id, sizeof(search_id), "TRN%08d%04d", rand() % (1000000 + num_operations_per_run), rand() % 10000); 
                search_transaction(&tmp_ht_for_ops, search_id);
            } else { // 20% Remocao (80-99)
                if (tmp_ht_for_ops.size > 0) {
                    char remove_id[16];
                    snprintf(remove_id, sizeof(remove_id), "TRN%08d%04d", rand() % (1000000 + num_operations_per_run), rand() % 10000); 
                    remove_transaction(&tmp_ht_for_ops, remove_id);
                }
            }
        }
        results[i] = stop_timer(&t);
        free_hash_table(&tmp_ht_for_ops); // Libera a tabela hash temporaria
    }

    double mean = calculate_mean(results, NUM_REPETITIONS);
    double std_dev = calculate_std_dev(results, NUM_REPETITIONS, mean);
    double cov = calculate_coeff_of_variation(mean, std_dev);

    printf("  Media: %.3f ms | DP: %.3f ms | CV: %.2f%%\n",
           mean, std_dev, cov);
}

// Executa todos os benchmarks completos para a HashTable
void run_all_benchmarks_hashtable(HashTable* ht_ptr) { 
    printf("\n===========================================\n");
    printf("=== INICIANDO BENCHMARKS COMPLETOS ===\n"); 
    printf("===========================================\n");

    // Limpa e inicializa a tabela hash antes de cada execucao completa
    free_hash_table(ht_ptr); 
    init_hash_table(ht_ptr); 

    // Preenche a tabela hash com dados iniciais para os benchmarks de busca, remocao e acesso
    printf("\nPreparando arvore para benchmarks (inserindo 10.000 elementos)... "); 
    generateRandomDataForHash(ht_ptr, 10000); 
    printf("Concluido. Tamanho da arvore: %d\n", ht_ptr->size); 

    printf("\n1. Tempo de Insercao:\n"); 
    benchmark_insertion_hashtable(1000); 
    benchmark_insertion_hashtable(10000); 

    printf("\n2. Tempo de Remocao:\n"); 
    benchmark_removal_hashtable(ht_ptr); 

    printf("\n3. Tempo de Busca:\n"); 
    benchmark_search_hashtable(ht_ptr); 

    printf("\n4. Uso de Memoria:\n"); 
    estimate_memory_usage_hashtable(ht_ptr); 

    printf("\n5. Tempo Medio de Acesso:\n"); 
    benchmark_random_access_hashtable(ht_ptr); 

    printf("\n6. Escalabilidade:\n"); 
    benchmark_scalability_hashtable();

    printf("\n7. Latencia Media (operacoes combinadas):\n"); 
    benchmark_combined_operations_hashtable(ht_ptr); 

    printf("\n===========================================\n");
    printf("=== BENCHMARKS CONCLUIDOS ===\n"); 
    printf("===========================================\n");
}

// Funcao para remover um elemento aleatorio da tabela hash (para restricao R2)
void remove_random_transaction(HashTable* ht) {
    if (ht->size == 0) return;

    // Encontra um indice aleatorio nao vazio na tabela
    int start_index = rand() % ht->capacity;
    int current_index = start_index;

    for (int k = 0; k < ht->capacity; k++) { 
        if (ht->table[current_index] != NULL) {
            // Remove o primeiro elemento da lista neste indice
            Transaction* temp = ht->table[current_index];
            ht->table[current_index] = temp->next;
            free(temp);
            ht->size--;
            return;
        }
        current_index = (current_index + 1) % ht->capacity;
    }
}

// Gera dados, incluindo anomalias e aplicando restricoes de tempo/tamanho
void generateAnomalousDataForHash(HashTable* ht, int count, int max_size) {
    for (int i = 0; i < count; i++) {
        // R2: Limitacao de tamanho (se a tabela hash atingir max_size, remove um elemento aleatorio)
        if (max_size > 0 && ht->size >= max_size) {
            remove_random_transaction(ht);
        }

        // R10: Interrupcoes periodicas (simula pausas a cada 100 iteracoes)
        if (i > 0 && i % 100 == 0) {
            Sleep(1); // Pausa de 1 milissegundo
        }

        Transaction d = {0}; 
        // Gera transaction_id unico como string
        snprintf(d.transaction_id, sizeof(d.transaction_id), "TRN%08d%04d", i + 2000000, rand() % 10000); 
        
        snprintf(d.timestamp, sizeof(d.timestamp), "2024-01-01 %02d:%02d:%02d", rand() % 24, rand() % 60, rand() % 60);
        snprintf(d.sender_account, sizeof(d.sender_account), "ACC%05d", rand() % 10000);
        snprintf(d.receiver_account, sizeof(d.receiver_account), "ACC%05d", rand() % 10000);
        d.amount = 10.0f + (rand() % 10000) / 100.0f;
        char types[][16] = {"purchase", "transfer", "deposit", "withdrawal"};
        snprintf(d.transaction_type, sizeof(d.transaction_type), "%s", types[rand() % 4]);
        char categories[][32] = {"retail", "food", "travel", "online", "services"};
        snprintf(d.merchant_category, sizeof(d.merchant_category), "%s", categories[rand() % 5]);
        snprintf(d.location, sizeof(d.location), "City%d", rand() % 100);
        char devices[][16] = {"mobile", "web", "pos"};
        snprintf(d.device_used, sizeof(d.device_used), "%s", devices[rand() % 3]);
        d.is_fraud = rand() % 2;

        // R18: Insercao de anomalias (10% de chance de inserir dados anomalos)
        if (rand() % 10 == 0) {
            snprintf(d.transaction_id, sizeof(d.transaction_id), "ANOMALY%06d", i + 1); // ID anomalo
            d.amount = -999.0f; // Valor anomalo
            snprintf(d.transaction_type, sizeof(d.transaction_type), "ANOMALY"); // Tipo anomalo
            d.is_fraud = true; // Marcar como fraude
        }

        insert_transaction(ht, d); // Insere na HashTable

        // R13: Delay artificial por lote (pausa a cada 100 transacoes inseridas)
        if (i > 0 && i % 100 == 0) {
            Sleep(50); // Pausa de 50 milissegundos
        }
    }
}

// Funcao para executar os benchmarks com restricoes para a HashTable
void run_restricted_benchmarks_hashtable() {
    printf("\n==============================================\n");
    printf("=== BENCHMARK COM RESTRICOES ATIVADAS ===\n"); 
    printf("==============================================\n");

    HashTable ht; 
    init_hash_table(&ht); 

    HighPrecisionTimer t;
    start_timer(&t);

    // R2, R10, R13, R18
    generateAnomalousDataForHash(&ht, 1000, 500); 

    double elapsed = stop_timer(&t);

    printf("\nTempo total (com 4 restricoes aplicadas): %.3f ms\n", elapsed);
    printf("Elementos finais na arvore (maximo 500): %d\n", ht.size); 

    // Benchmarks apos restricoes
    printf("\n--- Testes de Performance Pos-Restricoes ---\n"); 
    benchmark_search_hashtable(&ht); 
    benchmark_removal_hashtable(&ht); 
    benchmark_random_access_hashtable(&ht); 
    estimate_memory_usage_hashtable(&ht); 

    free_hash_table(&ht); 
    printf("\n==============================================\n");
    printf("=== FIM DOS TESTES COM RESTRICOES ===\n"); 
    printf("==============================================\n");
}


// ================= MAIN FUNCTION (BENCHMARK ONLY) =================

int main() {
    // Inicializa o gerador de numeros aleatorios com o tempo atual
    srand((unsigned int)time(NULL));

    // Cria e inicializa uma HashTable para benchmarks
    HashTable benchmark_hash_table; 
    init_hash_table(&benchmark_hash_table); 

    int choice;

    do {
        printf("\n--- Opcoes de Benchmark (Hash Table) ---\n"); 
        printf("1. Rodar Benchmarks Completos\n");
        printf("2. Rodar Benchmarks Restritos\n");
        printf("3. Sair\n");
        printf("Escolha uma opcao: ");
        scanf("%d", &choice);

        // Limpa o buffer de entrada
        while (getchar() != '\n'); 

        switch (choice) {
            case 1:
                run_all_benchmarks_hashtable(&benchmark_hash_table); 
                break;
            case 2:
                run_restricted_benchmarks_hashtable(); 
                break;
            case 3:
                printf("Saindo do programa de benchmark.\n");
                break;
            default:
                printf("Opcao invalida. Por favor, tente novamente.\n");
                break;
        }
    } while (choice != 3);

    // Libera a tabela hash principal de benchmark se ela ainda contiver dados
    free_hash_table(&benchmark_hash_table); 

    return 0;
}
